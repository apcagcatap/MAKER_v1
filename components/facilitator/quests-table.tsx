...
export function QuestsTable({
  ...
  // Fastest Completions Dialog State
  const [completionsDialogOpen, setCompletionsDialogOpen] = useState(false)
  const [selectedQuestForCompletions, setSelectedQuestForCompletions] = useState<Quest | null>(null)
  const [completionsData, setCompletionsData] = useState<Completion[]>([])
  ...
  const handleOpenCompletions = async (quest: Quest) => {
    setSelectedQuestForCompletions(quest)
    setCompletionsDialogOpen(true)
    ...
    try {
      ...
      if (error || !completionsData?.length) {
        setCompletionsData([])
        return
      }
      ...
      const mapped: Completion[] = completionsData.map((c: any) => {
        ...
        return {
          ...
          duration_seconds: c.completion_time_seconds ?? c.completion_time * 60,
        }
      })

      setCompletionsData(mapped)
    } catch (err) {
  ...
  const handlePublishQuest = async (questId: string) => {
    ...
    try {
      ...
      setQuests((prevQuests) =>
        prevQuests.map((q) => (q.id === questId ? { ...q, status: "Published" } : q))
      )
  ...
  return (
    <div className="min-h-screen bg-[#004A98]">
            ...
            <table className="w-full min-w-[1000px]">
              <thead>
                <tr className="bg-gray-100 border-b border-gray-200">
                  <th className="px-6 py-4 text-center text-sm font-semibold text-black" style={{ fontFamily: "Poppins, sans-serif" }}>Quest Name</th>
                  <th className="px-6 py-4 text-center text-sm font-semibold text-black" style={{ fontFamily: "Poppins, sans-serif" }}>Participants</th>
                  <th className="px-6 py-4 text-center text-sm font-semibold text-black" style={{ fontFamily: "Poppins, sans-serif" }}>Badge</th>
          ...
          {filteredQuests?.map((quest) => {
            ...
            return (
              <div key={quest.id} className="bg-white rounded-xl shadow-lg p-5 space-y-4 border border-gray-100">
                ...
                {/* Main Actions: Participants + Fastest */}
      ...
      {/* Fastest Completions Dialog */}
      <Dialog open={completionsDialogOpen} onOpenChange={setCompletionsDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Trophy className="w-5 h-5 text-yellow-500" />
              Fastest Completions — {selectedQuestForCompletions?.title}
            </DialogTitle>
          </DialogHeader>
          {completionsLoading ? (
            ...
            <FastestCompletions completions={completionsData} />
          )}
        </DialogContent>
      </Dialog>
    </div>
