...
export function FastestCompletions({ completions }: FastestCompletionsProps) {
  const medalIcons = [
    <Trophy className="w-5 h-5 text-yellow-500" key="gold" />,
    <Award className="w-5 h-5 text-gray-400" key="silver" />,
    <Medal className="w-5 h-5 text-amber-600" key="bronze" />
  ];

  return (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
      <h3 className="font-bold text-gray-900 mb-4">Fastest Completions</h3>
      ...
      {completions.length === 0 ? (
        ...
        <div className="space-y-4">
          {completions.map((completion, index) => (
            <div key={completion.user.id} className="flex items-center gap-3">
